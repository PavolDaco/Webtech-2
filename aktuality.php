    ...
    <div class="container">
        ...
        <form action="aktuality.php" method="GET" id="formular">
            ...
            <label style="margin-left: 10px" class="btn btn-default">
                <input type="radio" name="aktuality"
                       value="all" <?php if (isset($_GET['aktuality']) && $_GET['aktuality'] == 'all') echo ' checked="checked"'; ?>
                       checked/><?php echo $lang['AKT_VSETKY']; ?>
            </label>
            <label class="btn btn-default">
                <input type="radio" name="aktuality"
                       value="propagacia" <?php if (isset($_GET['aktuality']) && $_GET['aktuality'] == 'propagacia') echo ' checked="checked"'; ?> /><?php echo $lang['AKT_PROP']; ?>
            </label>
            <label class="btn btn-default">
                <input type="radio" name="aktuality"
                       value="oznamy" <?php if (isset($_GET['aktuality']) && $_GET['aktuality'] == 'oznamy') echo ' checked="checked"'; ?> /><?php echo $lang['AKT_OZNAMY']; ?>
            </label>
            <label class="btn btn-default">
                <input type="radio" name="aktuality"
                       value="zivot" <?php if (isset($_GET['aktuality']) && $_GET['aktuality'] == 'zivot') echo ' checked="checked"'; ?> /><?php echo $lang['AKT_ZOZIVOTA']; ?>
            </label>
            <label class="btn btn-primary">
            <input type="checkbox" id="old" name="old"
                   value="all" <?php if (isset($_GET['old']) && $_GET['old'] == 'all') echo ' checked="checked"'; ?> /><?php echo $lang['AKT_NEPLATNE']; ?>
            </label>
                <br>

            <button style="margin: 10px" class="form-control" type="submit" form="formular" value="Submit"><?php echo $lang['AKT_SHOW']; ?></button>
        </form>
