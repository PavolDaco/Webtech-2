...
echo "<table><tr><td id=\"column1_video\"><h3>" . $lang['PHOTO_PHOTO'] . "</h3>";

while (list($id, $title, $datum, $folder) = mysqli_fetch_row($result)) {
	echo "<br><a href=\"" . $_SERVER['PHP_SELF'] . "?id=" . $id . "\">$title</a>";

}
echo "</td><td id=\"column2_video\">";

$sql2 = $lang['PHOTO_SQL2'];
$sql2 = $sql2 . $addr . "'";

$result = $conn->query($sql2);

echo "<table id=\"table_photo_2\"><tr><td colspan=\"3\"  align=\"center\" id=\"nazov_galerie\"><h2>";
while (list($title, $datum, $folder) = mysqli_fetch_row($result)) {
...
for ($i=2; $i < $cislo ; $i++) { 
	echo "<div class=\"mySlides\" id=\"bimg" . ($i - 1) . "\"><div class=\"numbertext\">".($i - 1)." / ".($cislo - 2)."</div><img class=\"pokus\"src=\"intranet/photos_videos/photos/" . $direct . "/" . $files1[$i] . "\" style=\"width:100%\"></div>";
}
