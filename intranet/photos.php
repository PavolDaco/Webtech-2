...
if (isset($_SESSION['isReporter']) OR isset($_SESSION['isAdmin'])) { 
	...
	echo "<div id='okno_fotky' class='okno_profil'>
		...
		<form action='photos_videos/upload_photo.php' method='post' enctype='multipart/form-data'>
			...
			<input style='border-color:black;' class='btn btn-default' type='date' name='datum' placeholder='YYYY-MM-DD'><br>
			...
			<input style='border-color:black;' class='btn btn-default' type='text' name='TitleEN' placeholder='Názov (EN)'><br><br>
			
			<label for='fileToUpload[]'>Vyber obrázky: (max 50, max 2MB):</label><br>
		    <input style='border-color:black;' class='btn btn-default' type='file' name='fileToUpload[]' id='fileToUpload' multiple='multiple'><br>
		    <input style='border-color:black;' class='btn btn-default' type='submit' value='Nahraj obrázky' name='submit'>
		</form>
		<hr>";
	...
	echo "<p><strong>Pridanie fotiek do existujúcej galérie</strong></p>
		<form action='photos_videos/upload_photo.php' method='post' enctype='multipart/form-data'>
			<input style='border-color:black;' class='btn btn-default' type='checkbox' checked='true' name='add' style='display:none;' />
